...
public class PointLight implements Light
{
	 ...
	 *            Position
	 * @param coefficient
	 *            Attenuation formula coefficients
	 * @param color
	 *            Light color
	 * @param intensity
	 *            Light intensity
	...
	{
		if (Math.abs(Camera.getPos().subtract(pos).length()) > Math.abs(Camera.getPos().subtract(((PointLight) l).getPos()).length())) { return 1; }

		if (Math.abs(Camera.getPos().subtract(pos).length()) < Math.abs(Camera.getPos().subtract(((PointLight) l).getPos()).length())) { return -1; }

		if (Math.abs(Camera.getPos().subtract(pos).length()) == Math.abs(Camera.getPos().subtract(((PointLight) l).getPos()).length())) { return 0; }
