...
public class ObjectLoader
{
	public static List<Vertex> loadOBJ(String path)
	{
		...
		try
		{
			...
			while(file.hasNext())
			{
				...
				/*
				 * I couldn't tell you why this one is group(1). It just works. Regular expressions are weird.
				 */
				...
				if(line.startsWith("f"))
				{
					...
					int[] faces = new int[(uv && normal ? 9 : uv || normal ? 6 : 3)];
					...
					{
						if(!normal && !uv)
							faceVertices.add(new Vertex(positions.get(faces[mul*j] - 1)));
						if(normal && !uv)
							faceVertices.add(new Vertex(positions.get(faces[mul*j] - 1), normals.get(faces[j*mul+1] - 1)));
						if(!normal && uv)
							faceVertices.add(new Vertex(positions.get(faces[mul*j] - 1), uvs.get(faces[j*mul+1] - 1)));
						if(normal && uv)
	...
	public static String[] removeEmptyStrings(String[] data)
	{
		...
		for(int i = 0; i < data.length; i++)
			{
				if(!data[i].equals(" ") && !data[i].equals("") && !data.equals("  "))
					{
						result.add(data[i]);
					}
			}
	...
	/**
	 * Mostly benny's code. Github at https://github.com/BennyQBD/GDXJ/blob/master/src/com/base/engine/Mesh.java
